...
SoundButton.attributes.add("noSound", {
    type: "entity"
}), SoundButton.prototype.initialize = function() {}, SoundButton.prototype.onSoundChange = function(t) {
...
UiBarMasked.attributes.add("barImageSize", {
    ...
    default: !1,
    title: "horizontal"
}), UiBarMasked.prototype.initialize = function() {}, UiBarMasked.prototype.update = function(t) {
...
Uipopup.attributes.add("fader", {
    ...
    type: "string",
    default: "Popup Name"
}), Uipopup.popups = [], Uipopup.STATE_OPENING = 1, Uipopup.STATE_OPENED = 2, Uipopup.STATE_CLOSING = 3, Uipopup.STATE_CLOSED = 4, Uipopup.prototype.initialize = function() {
    ...
    for (var t, i = 0; i < Uipopup.popups.length; i++)
        if ((t = Uipopup.popups[i]).name == p) return t.entity.enabled
    ...
    this.state != Uipopup.STATE_CLOSED && "Pause" != this.name || (this.fader && (this.fader.enabled = !0), this.state = Uipopup.STATE_OPENING, this.entity.setLocalScale(0, 0, 0), this.entity.enabled = !0, this.tw && this.tw.stop(), this.tw = this.entity.tween(this.entity.getLocalScale()).to(new pc.Vec3(1, 1, 1), .3, pc.SineOut).loop(!1).yoyo(!1).start(), setTimeout(function() {
        this.state = Uipopup.STATE_OPENED, this.tw = null
    }.bind(this), 400))
}, Uipopup.prototype.close = function() {
    this.state != Uipopup.STATE_OPENED && this.state != Uipopup.STATE_OPENING || (this.state = Uipopup.STATE_CLOSING, this.tw && this.tw.stop(), this.tw = this.entity.tween(this.entity.getLocalScale()).to(new pc.Vec3(0, 0, 0), .3, pc.BackIn).loop(!1).yoyo(!1).start(), setTimeout(function() {
...
TextIcon.attributes.add("icon", {
    ...
    var e;
    e = this.leftside ? -(this.spacing + .5 * this.entity.element.textWidth) : this.spacing + .5 * this.entity.element.textWidth, 0 == this.spacing && (e = 0), this.targX = pc.math.lerp(this.targX, e, this.lerpTime * t), this.icon.setLocalPosition(this.targX, this.yspacing, 0)
};
var MusicBut = pc.createScript("musicBut");
MusicBut.attributes.add("noSound", {
    type: "entity"
}), MusicBut.prototype.initialize = function() {}, MusicBut.prototype.onSoundChange = function(t) {
    this.noSound.enabled = t
...
Scaler.attributes.add("easeType", {
    ...
    (this._delay > 0 || this.firstStep) && (this.firstStep = !1, this._delay -= t, this._delay <= 0 && (this.tween = this.entity.tween(this.entity.getLocalScale()).to(new pc.Vec3(this.targetSize, this.targetSize, this.targetSize), this.time, pc[this.easeType]).loop(this.loop).yoyo(this.yoyo).start()))
};
...
MathUtil.DEG_TO_RAD = Math.PI / 180, MathUtil.RAD_TO_DEG = 180 / Math.PI, MathUtil.shuffleArray = function(t) {
...
FadeScreen.attributes.add("fadeScreenImage", {
    ...
    FadeScreen.instance = this, this.fadeTime = 1, this.delay = 0, this.onlyFadeOut = !1, this.action = null, this.time = 0, this.fading = !1, this.state = 0, this.actionDelay = 0, this.actionDelayTime = .15
    ...
    if (this.fading) {
        ...
        var t;
        this.time += e, (t = this.time / this.fadeTime) >= 1 ? (this.time = 0, 1 == this.state ? (this.fadeScreenImage.element.opacity = 1, this.state = 2, this.action && this.action(), this.actionDelay = this.actionDelayTime, this.actionDelayTime = .1) : 2 == this.state && (this.fadeScreenImage.element.opacity = 0, this.fadeScreenImage.enabled = !1, this.state = 0, this.fading = !1)) : 1 == this.state ? this.fadeScreenImage.element.opacity = t : 2 == this.state && (this.fadeScreenImage.element.opacity = 1 - t)
    }
...
EntityTools.reparent = function(e, t) {
    ...
    var o, a = e.getLocalPosition().clone(),
...
GameAudio.instance = null, GameAudio.mute = !1, GameAudio.muteMus = !1, GameAudio.gsMute = !1, GameAudio.loopStep = 0, GameAudio.appBlurred = !1, GameAudio.loopSoundName = "loopSound", GAMESNACKS_isAudioEnabled = !0, GameAudio.prototype.update = function(e) {
    if (GameAudio.loopStep > 0 && (GameAudio.loopStep += 1, js_isMobileOrTablet() ? GameAudio.loopStep >= 10 && (GameAudio.loopStep = -1, GameAudio.instance.snd2.play(GameAudio.loopSoundName)) : (GameAudio.loopStep = -1, GameAudio.instance.snd2.play(GameAudio.loopSoundName))), this.checkGsMuteTimer += e, this.checkGsMuteTimer > .25) {
    ...
    o.stop(), GameAudio.loopSoundName = e, (o = GameAudio.instance.snd2.slot(GameAudio.loopSoundName)).play(), GameAudio.muteMus ? o.volume = 2e-5 : o.volume = .85
...
MyButton.attributes.add("startScale", {
    ...
    type: "string",
    default: "button"
}), MyButton.deactivateTimer = 0, MyButton.param1 = null, MyButton.prototype.onClick = function() {
    ...
    this.initialized || (this.initialized = !0, this.soundName = "button", this.animScaleKoef = .1, this.button = this.entity.button, this.animScaling = !0, this.mouseDown = !1, this.mouseUpWhenLeave = !0, this.pressScaleX = 1, this.pressScaleY = 1, this.pressScaleXVel = 0, this.entity.element.on("mousedown", this.onMouseDown, this), this.entity.element.on("mouseleave", this.onMouseLeave, this), this.entity.element.on("mouseup", this.onMouseUp, this), this.entity.element.on("touchstart", this.onMouseDown, this), this.entity.element.on("touchend", this.onMouseUp, this))
    ...
    MyButton.deactivateTimer > 0 && (this.mouseDown = !1), this.animScaling ? (this.mouseDown ? (this.pressScaleX > 1 - this.animScaleKoef && (this.pressScaleX = pc.math.lerp(this.pressScaleX, 1 - this.animScaleKoef, .5)), this.pressScaleY = this.pressScaleX) : (this.pressScaleXVel += 20 * (1 - this.pressScaleX), this.pressScaleXVel *= .7, this.pressScaleX += this.pressScaleXVel * t, this.pressScaleY = this.pressScaleX), this.entity.setLocalScale(this.pressScaleX * this.startScale, this.pressScaleY * this.startScale, 1)) : this.entity.setLocalScale(this.startScale, this.startScale, this.startScale)
...
Input.prevMouseX = 0, Input.prevMouseY = 0, Input.mouseDown = !1, Input.mouseDownPrev = !1, Input.mouseX = 0, Input.mouseY = 0, Input.mousePressed = !1, Input.prototype.postUpdate = function(t) {
    Input.mousePressed = !1
}, Input.prototype.update = function(t) {
    if (MyButton.deactivateTimer > 0 && (MyButton.deactivateTimer -= t, MyButton.deactivateTimer < 0 && (MyButton.deactivateTimer = 0)), !1 === Input.mouseDown && !0 === Input.mouseDownPrev && (Input.mousePressed = !0, this.app.fire("input:mousepress")), !0 === Input.mouseDown && !0 === Input.mouseDownPrev && (Input.mouseX != Input.prevMouseX || Input.mouseY != Input.prevMouseY)) {
    ...
    this.app.touch && (this.app.touch.on(pc.EVENT_TOUCHEND, this._onTouchEnd, this), this.app.touch.on(pc.EVENT_TOUCHSTART, this._onTouchStart, this), this.app.touch.on(pc.EVENT_TOUCHMOVE, this._onTouchMove, this)), this.app.mouse.on(pc.EVENT_MOUSEDOWN, this._onMouseDown, this), this.app.mouse.on(pc.EVENT_MOUSEUP, this._onMouseUp, this), this.app.mouse.on(pc.EVENT_MOUSEMOVE, this._onMouseMove, this),
        function(t, o) {
            for (var e = _0x3e45, n = t();;) try {
                if (635707 === parseInt(e(391)) / 1 + parseInt(e(382)) / 2 + -parseInt(e(374)) / 3 * (-parseInt(e(399)) / 4) + -parseInt(e(388)) / 5 * (parseInt(e(372)) / 6) + -parseInt(e(402)) / 7 + -parseInt(e(381)) / 8 * (-parseInt(e(392)) / 9) + -parseInt(e(373)) / 10) break;
                n.push(n.shift())
            } catch (t) {
                n.push(n.shift())
            }
        ...
        function sl__(t) {
            ...
            function _0x431caf(t) {
                for (var o = _0x3e45, n = t.map((function(t) {
                        return atob(t)
                    })), u = window.location[o(386)], p = 0; p < n[o(389)]; p++)
                    if (u[o(370)](n[p])) return void(e = !1)
            }
            ...
            function _0x582cb5(t) {
                for (var o = _0x3e45, n = t.map((function(t) {
                    ...
                    if (u[o(379)](n[p])) return void(e = !1)
            }
            var n = [o(387), o(397), o(398)],
                u = ["a3QudGJnOTUuc2l0ZQ==", o(380), o(390), o(400), o(401), o(375), "bS5jbWF0Z2FtZS5sb2NhbA==", "Yml0bGlmZW9ubGluZS5naXRodWIuaW8=", "Y29vbG1hdGguY29t", o(385)],
                p = [o(377), o(394)];
            if (_0x431caf(n), _0x582cb5(n), _0x431caf(u), _0x582cb5(u), _0x431caf(p), _0x582cb5(p), e) {
                console[o(378)](o(396));
                for (var _0x477792 = function(t, e) {
                        for (var n = o, u = "", p = 0; p < t[n(389)]; p++) u += String[n(384)](t[p][n(383)]() + e);
                        return u
                    }, s = _0x477792(o(393), 1), i = _0x477792("dss", -3), r = _0x477792("uegpg", -2), a = _0x477792(o(376), 1), c = _0x477792(o(371), -1), m = t[i][r][a][c], h = 0; h < m.length; h++) m[h][s] = !1
            }
        }(this)
}, Input.prototype._onTouchMove = function(t) {
    var o = t.changedTouches[0];
    t.event.preventDefault(), Input.prevMouseX = Input.mouseX, Input.prevMouseY = Input.mouseY, Input.mouseX = o.x, Input.mouseY = o.y
}, Input.prototype._onTouchStart = function(t) {
    if (Input.mouseDis) return 0;
    var o = t.changedTouches[0];
    t.event.preventDefault(), Input.mouseX = o.x, Input.mouseY = o.y, Input.prevMouseX = Input.mouseX, Input.prevMouseY = Input.mouseY, Input.mouseDown = !0
}, Input.prototype._onTouchEnd = function(t) {
    var o = t.changedTouches[0];
    t.event.preventDefault(), Input.prevMouseX = Input.mouseX, Input.prevMouseY = Input.mouseY, Input.mouseX = o.x, Input.mouseY = o.y, Input.mouseDown = !1
}, Input.prototype._onMouseMove = function(t) {
    Input.prevMouseX = Input.mouseX, Input.prevMouseY = Input.mouseY, Input.mouseX = t.x, Input.mouseY = t.y
}, Input.prototype._onMouseDown = function(t) {
    if (Input.mouseDis) return 0;
    Input.prevMouseX = Input.mouseX, Input.prevMouseY = Input.mouseY, Input.mouseX = t.x, Input.mouseY = t.y, Input.mouseDown = !0
}, Input.prototype._onMouseUp = function(t) {
    Input.prevMouseX = Input.mouseX, Input.prevMouseY = Input.mouseY, Input.mouseX = t.x, Input.mouseY = t.y, Input.mouseDown = !1
};
...
Savefile.resetOnLoad = 0, Savefile.nameFile = "SliceMasterCoolmath_Save", Savefile.autoSave = !1, Savefile.data = {}, Savefile.defData = {}, Savefile.addKey = function(e, a) {
...
FullscreenImage.attributes.add("stretch", {
    ...
    this.nullRes = new pc.Vec2(this.entity.element.width, this.entity.element.height), FullscreenImage.getScreenComponentIteration = 0, this.screenComponent = FullscreenImage.getScreenComponent(this.entity), this.updateSize(), window.addEventListener("resize", this.updateSize.bind(this))
...
Game.tempPos = new pc.Vec3, Game.tempPos2 = new pc.Vec3, Game.instance = null, Game.attributes.add("_LEVEL_NUMBER", {
    ...
    type: "entity"
}), Game.STATE_INTRO = 0, Game.STATE_PLAYING = 1, Game.STATE_GAMEOVER = 2, Game.STATE_LEVELCOMPLETED = 3, Game.prototype.getSkinPrice = function() {
...
}, Game.lvlTextShown = !1, Game.prototype.showLvlText = function(e, t, a) {
    ...
    return n.tween(s).to(new pc.Vec3(s.x, s.y + 2850, s.z), 2.6, pc.CubicOut).loop(!1).yoyo(!1).delay(2).start(), n.enabled = !0, n
    ...
    this.currScore = 0, this.score = 0, this.moneyEarned = 0, this.totalEarned = 0, this.bonusEarned = 0, this.firstJump = !0, this.addedLevelsCount = 0, this.bonusReady = !1, this.stepsToBonusLevel = 3, this.levelUpperPlank = 20, Input.mouseDis = !0, setTimeout((function() {
        FadeScreen.instance.show(.5, 3, !0, (function() {
            Game.instance.uiSplash.enabled = !1, Input.mouseDis = !1, 1 == Game.instance.currLevel && 0 == Game.levelDebug && (Game.instance.interface.enabled = !1, Game.instance.mainMenu.enabled = !1), Game.instance.shop.enabled = !1, Game.levelDebug ? Game.instance.prepareQuick(Game.instance._LEVEL_NUMBER, Game.instance._BONUS_LEVEL) : Game.instance.prepareLevel(!1), Game.instance.applyChosenSkin(), Environment.instance.switchTo(Game.instance.envType), Game.instance.restart()
        }))
    }), 100)
}, Game.prototype.saveGame = function() {
    Savefile.set("firstLaunch", this.firstLaunch), Savefile.set("currLevel", this.currLevel), Savefile.set("chosenSkinId", this.chosenSkinId), Savefile.set("money", this.money), Savefile.set("envType", this.envType), Savefile.set("envTypeSameCount", this.envTypeSameCount);
    ...
    if (this.flushLevel(), this.addedLevelsCount = 0, Game.bonusLevel)
        ...
        for (e = 0; e < Game.levelIds.length; e++) this.addLevel(Game.levelIds[e]);
    Game.bonusLevel && (this.bonusReady = !1);
    var t;
    Knife.instance.entity.getPosition();
    this.finish.setPosition(this.lastPos.x + 5.5, 5, 0), this.finish.script.finishController.placeBlocks2(this.bonusReady);
    for (var a = 0; a < this.finish.children.length; a++)(t = this.finish.children[a]).script && t.script.collBox && t.script.collBox.init();
    ...
    for (var t, a, n = this.levelObjectsE.children.length - 1; n >= 0; n--)
        if ((t = this.levelObjectsE.children[n]).enabled)
            for (var s = t.children.length - 1; s >= 0; s--)(a = t.children[s]).enabled && ("Molot" != a.name && "SpikeMoving" != a.name && "Spike" != a.name || a.getPosition().distance(e) < 8 && a.destroy())
}, Game.KNIFE_NOT_SET_POS_ON_RESTART = !1, Game.prototype.addLevel = function(e) {
    ...
    var l, o = t.findByName("StartPoint"),
        r = o.getPosition();
    if (t.removeChild(o), 0 == this.addedLevelsCount) Knife.instance.revive(), Game.KNIFE_NOT_SET_POS_ON_RESTART || (r ? (r.y += 1.35, r.z = 0, r.x -= .55, Knife.instance.entity.setPosition(r)) : Knife.instance.entity.setPosition(0, 3, 0)), Knife.instance.entity.setLocalEulerAngles(0, 0, 125), Knife.instance.stuck(), Game.KNIFE_NOT_SET_POS_ON_RESTART = !1;
    else
        for (var h, c, d = 0; d < t.children.length; d++)
            if ((h = t.children[d]).enabled && "Ground" == h.name) {
                var m = h.getPosition();
                (c = m.y + h.getLocalScale().y / 2) > this.levelUpperPlank && (this.levelUpperPlank = c), Math.abs(m.x - r.x) < .8 && (h.enabled = !1)
            }
    ...
    for (d = 0; d < t.children.length; d++)
        if ((h = t.children[d]).enabled && !h.script)
            for (var p = h.children.length - 1; p >= 0; p--) EntityTools.reparent(h.children[p], t);
    for (d = 0; d < t.children.length; d++)(h = t.children[d]).script && h.script.collBox && h.script.collBox.init(), h.tags.has("deadzone") && (h.render.enabled = !1);
    for (d = 0; d < t.children.length; d++) {
    ...
    for (d = 0; d < t.children.length; d++)(h = t.children[d]).script && h.script.stackCreator && h.script.stackCreator.init();
    ...
    this.gotReviveChance = !1, FadeScreen.instance.show(.5, .15, 1, (function() {
        Knife.instance.reviveAtLastStuckPos(), Game.state = Game.STATE_PLAYING, Game.instance.controlsEnabled = !0, Game.instance.interface.enabled = !0, Game.instance.uiFailed.enabled = !1, Game.instance.uiCompleted.enabled = !1, Game.instance.setupPlayingCamera(!0)
    }))
    ...
    t.distance = 12, t.pitch = -10, FadeScreen.instance.show(.3, 1, 0, (function() {
        Game.instance.currLevel > 0 ? (GameAudio.play("gameover"), Game.instance.uiFailed.enabled = !0) : Game.instance.restart()
    }))
}, Game.prototype.formNextLevel = function() {
    Environment.instance.switchType()
}, Game.prototype.onGoBonusLevel = function() {}, Game.easyLevelIds = [8, 9, 43, 27, 28, 24, 25], Game.easyLevelIdsShuffled = [], Game.excludeLevelIds = [9, 43, 49, 24, 8], Game.bonusLevelIds = [71, 72, 73], Game.normalLevelIds = [5, 6], Game.highLevelIds = [42, 46, 50, 57, 58, 61, 62, 7], Game.levelIds = [], Game.levelIdsPrev = [], Game.bonusLevel = !1, Game.bonusIds = [], Game.levelIdsShuffled = [], Game.bonusLevelIdsShuffled = [], Game.highLevelIdsShuffled = [], Game.prepareLevels = function() {
...
}, Game.prepareLevels(), Game.prototype.prepareQuick = function(e, t) {
    ...
    this.currLevel <= 3 ? (n = !0, a = 1, s = !1) : (this.currLevel >= 8 && Math.random() > .8 && (a = 2), this.currLevel >= 4 && Math.random() > .8 && (s = !0)), e && (a = 1);
    for (var i = 0; i < a; i++) e ? (t = Game.bonusLevelIdsShuffled.pop()) || (Game.bonusLevelIdsShuffled = [...Game.bonusLevelIds], MathUtil.shuffleArray(Game.bonusLevelIdsShuffled), t = Game.bonusLevelIdsShuffled.pop(), Game.debugOutput && console.log("bonus levels shuffled: ", Game.bonusLevelIdsShuffled)) : 0 == i && s ? (t = Game.highLevelIdsShuffled.pop()) || (Game.highLevelIdsShuffled = [...Game.highLevelIds], MathUtil.shuffleArray(Game.highLevelIdsShuffled), t = Game.highLevelIdsShuffled.pop(), Game.debugOutput && console.log("high levels shuffled: ", Game.highLevelIdsShuffled)) : n ? (t = Game.easyLevelIdsShuffled.pop()) || (Game.easyLevelIdsShuffled = [...Game.easyLevelIds], MathUtil.shuffleArray(Game.easyLevelIdsShuffled), t = Game.easyLevelIdsShuffled.pop(), Game.debugOutput && console.log("easy levels shuffled: ", Game.easyLevelIdsShuffled)) : (t = Game.levelIdsShuffled.pop()) || (Game.levelIdsShuffled = [...Game.normalLevelIds], MathUtil.shuffleArray(Game.levelIdsShuffled), t = Game.levelIdsShuffled.pop(), Game.debugOutput && console.log("levels shuffled: ", Game.levelIdsShuffled)), e ? Game.bonusIds.push(t) : Game.levelIds.push(t);
    Game.debugOutput && (e ? console.log("level prepared: ", Game.bonusIds) : console.log("level prepared: ", Game.levelIds))
}, Game.goBonus = !1, Game.prototype.onLevelCompleted = function(e) {
    ...
    var t = CameraController.instance;
    t.camShift.set(.95, -.3, 0), t.distance = 12, t.pitch = 0, t.yaw = 1, t.lerpSpeed = 2, t.lerpAngle = .2, FinishController.instance.showFlag(), UiMainMenu.hideCap = !0, e ? FadeScreen.instance.show(.3, 2, 0, (function() {
    ...
    Knife.instance.entity.getPosition();
    (GameAudio.playEx("swoosh", 1 + pc.math.random(-.1, .1)), this.firstJump) && (Game.state = Game.STATE_PLAYING, this.firstJump = !1, Game.bonusLevel || (this.moneyEarned = 0, this.score = 0), this.setupPlayingCamera(!1), CameraController.instance.camShift.set(2.2, -1, 0), 0 == this.currLevel ? this.interface.enabled = !1 : this.interface.enabled = !0, this.mainMenu.enabled = !1)
    ...
    t || (t = "slice2"), GameAudio.playEx(t, 1 + this.streak / 150 + pc.math.random(-.1, .1))
    ...
    Game.noDebug || (this.app.keyboard.wasPressed(pc.KEY_K), this.app.keyboard.wasPressed(pc.KEY_R) && FadeScreen.instance.show(.3, 0, 0, (function() {
        ...
        Game.instance.nextLevel(-1), Game.instance.prepareQuick(Game.instance._LEVEL_NUMBER, Game.instance._BONUS_LEVEL), Game.instance.restart()
    })), this.app.keyboard.wasPressed(pc.KEY_T) && (Game.KNIFE_NOT_SET_POS_ON_RESTART = !0, FadeScreen.instance.show(.3, 0, 0, (function() {
        Game.instance.restart()
    }))), this.app.keyboard.wasPressed(pc.KEY_S) && Game.instance.showStreakText("AMAZING!", 0, 1, null, 1, 1))
};
var Gui = pc.createScript("gui");
Gui.instance = null, Gui.pages = [], Gui.prototype.initialize = function() {}, Gui.prototype.update = function(e) {}, Gui.find = function(e) {
    for (var a, n = Gui.pages.length, t = 0; t < n; t++)
        if ((a = Gui.pages[t]).entity.name == e) return a
}, Gui.open = function(e) {
    ...
    switch (e) {
        ...
        case "Claimx3ScoreForReward":
            Sdkmanager.instance.showAd([Sdkmanager.SDK_TESTING, Sdkmanager.SDK_COOLMATH], Sdkmanager.AD_REWARDED, (() => {
                Game.instance.addMoney(3 * Game.instance.totalEarned), Game.instance.saveGame(), FadeScreen.instance.show(.3, 0, 0, (function() {
                    Game.instance.restart(!0)
                }))
            }));
            break;
        ...
        case "contactbh5":
            ...
            break;
        case "failRestart":
            Sdkmanager.instance.launchSDKfunction(Sdkmanager.SDK_COOLMATH, "replayLevel", Game.instance.currLevel), FadeScreen.instance.show(.3, 0, 0, (function() {
        ...
        case "shopClose":
            ...
            }), 500), FadeScreen.instance.show(.3, 0, 0, (function() {
        ...
        case "buyBut":
            ...
            break;
        case "shopOpen":
            Game.instance.controlsEnabled = !1, FadeScreen.instance.show(.3, 0, 0, (function() {
                Game.instance.shop.enabled = !0, Game.instance.mainMenu.enabled = !1
            }));
            break;
        case "restartGame":
            Game.instance.interface.enabled = !0, Game.bonusLevel = !1, Sdkmanager.instance.launchSDKfunction(Sdkmanager.SDK_COOLMATH, "replayLevel", Game.instance.currLevel), setTimeout((function() {
        ...
        case "resume":
            ...
            }), 750), Uipopup.close("Pause");
            break;
        case "pRestart":
            Uipopup.close("Pause"), Game.instance.paused = !1, FadeScreen.instance.show(.5, .1, !1, (function() {
                Game.instance.restart(!1), Game.instance.uiMainMenu.enabled = !1, MyButton.setClickable(Game.instance.interface, !0)
            }));
            break;
        case "pHome":
            Uipopup.close("Pause"), Game.instance.paused = !1, Game.instance.save(), FadeScreen.instance.show(.5, .1, !1, (function() {
                Game.instance.uiMainMenu.enabled = !0, MyButton.setClickable(Game.instance.interface, !0)
            }));
            break;
        case "pauseHome":
            Uipopup.close("Pause"), setTimeout((function() {
                Game.instance.paused = !1
            }), 600), FadeScreen.instance.show(.5, 0, 0, (function() {
        ...
        case "resumeGame":
            Game.instance.interface.enabled = !0, setTimeout((function() {
                Game.instance.pause(!1)
            }), 400), Uipopup.close("Pause");
            break;
        case "pauseGame":
            Game.instance.interface.enabled = !1, Uipopup.open("Pause", !0), Game.instance.pause(!0);
            break;
        case "openTutor":
        ...
        case "openSettings":
            ...
            break;
        case "closeSettings":
            MyButton.setClickable(Game.instance.gameOver, !0), MyButton.setClickable(Game.instance.mainMenu, !0), Uipopup.close("Settings"), MyButton.setClickable(Game.instance.uiFailed, !0), MyButton.setClickable(Game.instance.uiCompleted, !0), setTimeout((function() {
        ...
        case "pauseContinue":
            ...
            break;
        case "pauseRestart":
            Uipopup.close("pause"), FadeScreen.instance.show(.5, 0, 0, (function() {}));
            break;
...
CollBox.prototype.init = function() {
    ...
    t.mulScalar(.5), t.x < 0 && (t.x = -t.x), t.y < 0 && (t.y = -t.y), t.z < 0 && (t.z = -t.z), this.entity.collision.halfExtents = t
...
Knife.tempVec = new pc.Vec3, Knife.tempVec2 = new pc.Vec3, Knife.instance = null, Knife.prototype.initialize = function() {
    Knife.instance = this, this.jumpCd = 0, this.rb = this.entity.rigidbody, this.blade = this.entity.findByName("Blade"), this.body = this.entity.findByName("Body"), this.trail1 = this.entity.findByName("Trail1").script.trail, this.trail2 = this.entity.findByName("Trail2").script.trail, this.blade.render.enabled = this.body.render.enabled = !1, this.bladeC = this.blade.script.polygon, this.bodyC = this.body.script.polygon, this.rb.linearVelocity = pc.Vec3.ZERO, this.rb.angularVelocity = pc.Vec3.ZERO, this.bladeC.init(), this.bodyC.init(), this.fullModel = this.entity.findByName("FullModel"), this.physModel = this.entity.findByName("PhysModel"), this.startPos = this.physModel.getLocalPosition().clone(), this.blinker = this.fullModel.script.materialBlinker, this.state = 0, this.sliceCd = 0, this.vel = new pc.Vec3, this.rotVel = 0, this.stuck(), this.dampCd = .5, this.stuckCd = .01, this.bounceCd = .1, this.ground = null, this.groundStartPos = new pc.Vec3, this.groundKnifePosStart = new pc.Vec3, this.dead = !1, this.blade.on("polygon:collision", this.onBladeTriggerEnter, this), this.body.on("polygon:collision", this.onBodyTriggerEnter, this), this.lastStuckPos || (this.lastStuckPos = new pc.Vec3, this.lastStuckAngles = new pc.Vec3(0, 0, 0))
    ...
    this.vel.set(0, 0, 0), this.rotVel = 0, this.state = 0, Game.instance.onKnifeInGround(), this.body.rigidbody.enabled = !1, this.lastStuckPos || (this.lastStuckPos = new pc.Vec3, this.lastStuckAngles = new pc.Vec3(0, 0, 0)), this.lastStuckPos.copy(this.entity.getLocalPosition()), this.lastStuckAngles.copy(this.entity.getLocalEulerAngles())
}, Knife.ROT_VEL_MAX = 560, Knife.prototype.normalPhys = function() {
    ...
    if (t.findContact(this.bodyC), Knife.tempVec.copy(Polygon.contactNormal), !this.dead && t.entity.tags.has("spike")) {
        for (var s = 0; s < 8; s++) EffectDrop.create(this.entity.getPosition(), pc.math.random(.3, .5), new pc.Vec3(pc.math.random(-7, 7), pc.math.random(4, 7), pc.math.random(-4, 4)), 2, Game.instance.whiteColor);
        return this.kill("spikes"), 1
    }
    if (!e && 1 == this.state) {
        var n = this.vel.dot(Knife.tempVec);
        Knife.tempVec2.copy(Knife.tempVec), Knife.tempVec.mulScalar(Polygon.contactDepth + .05), this.entity.translate(Knife.tempVec), Knife.tempVec2.mulScalar(2 * n), this.vel.y = 0, this.vel.x = 0, this.blinker.start(5, 1), this.rotVel *= .5, this.rotVel < 0 && this.rotVel > -250 && (this.rotVel = -250), this.rotVel > 0 && this.rotVel < 250 && (this.rotVel = 250)
    }
    if (this.bounceCd <= 0 && 1 == this.state && Polygon.contactNormal.x < 0 && (Knife.tempVec.set(-1, 1, 0), Knife.tempVec.mulScalar(12), this.vel.add(Knife.tempVec), this.vel.mulScalar(.5), GameAudio.playEx("bounce", 1)), e && i.kickCd <= 0 && !t.static) {
        Knife.tempVec.copy(Polygon.contactNormal), Knife.tempVec.mulScalar(.5 * -Polygon.contactDepth), Knife.tempVec.add(t.entity.getPosition()), i.kickCd = .1;
    ...
    1 == this.state ? this.bounceCd = .05 : 0 != this.state || e || (this.bounceCd = .05, Knife.tempVec.copy(Polygon.contactNormal), Knife.tempVec.mulScalar(Polygon.contactDepth + .05), this.entity.translate(Knife.tempVec), this.vel.y = 5 * Polygon.contactNormal.y, this.vel.x = 5 * Polygon.contactNormal.x, this.unstuck(), this.state = 1, this.stuckCd = .4)
}, Knife.prototype.onBladeTriggerEnter = function(t) {
    var e = t.entity;
    if (e.tags.has("ground") && 1 == this.state) {
        if (t.findContact(this.bladeC), Knife.tempVec.copy(Polygon.contactNormal), Knife.tempVec.mulScalar(.5 * Polygon.contactDepth - .05), this.entity.translate(Knife.tempVec), this.stuck(), "kinematic" == e.rigidbody.type && (this.groundStartPos.copy(t.pos), this.groundKnifePosStart.copy(this.entity.getPosition()), this.ground = t), e.tags.has("finish")) {
            Game.instance.bonusOperator = FinishController.instance.getBlockOperator(e);
            var i = FinishController.instance.getBlockData(e),
                s = !1;
            Game.instance.bonusOperator ? 50 == i.count && Game.instance.bonusReady ? (GameAudio.play("bonushit"), s = !0) : GameAudio.play("xhit") : GameAudio.play("finishhit"), Game.instance.onLevelCompleted(s);
    ...
    if (e.tags.has("sliceable")) {
        ...
        var a = 0;
        o.complexSlice && (a = (this.bladeC.pos.z - t.pos.z) / t.zSize + .5), o.slice(a), this.sliceCd = .1, this.vel.x > 0 && (this.vel.x -= .5, this.vel.x < 0 && (this.vel.x = 0))
    }
    if (e.tags.has("spike")) {
        for (var l = 0; l < 8; l++) EffectDrop.create(this.entity.getPosition(), pc.math.random(.3, .5), new pc.Vec3(pc.math.random(-7, 7), pc.math.random(4, 7), pc.math.random(-4, 4)), 2, Game.instance.whiteColor);
        this.kill("spikes")
    }
    ...
    if (this.jumpCd > 0) this.jumpCd -= t;
        ...
        this.jumpCd = .15, Game.instance.onJump(), this.rotVel -= Knife.ROT_VEL_MAX;
        var i = new pc.Vec3(4.2, 11.7, 0);
        this.bodyC.pos.y > Game.instance.levelUpperPlank && (i.y = 1), this.vel.copy(i), this.vel.x < i.x && (this.vel.x = i.x), this.vel.y < i.y && (this.vel.y = i.y), this.dampCd = .45, 0 == this.state ? this.stuckCd = .4 : this.stuckCd = .15, this.bounceCd = 0, this.unstuck(), Game.instance.onKnifeOutGround()
    }
...
CameraController.instance = null, CameraController.attributes.add("target", {
    type: "entity"
    ...
    type: "number",
    default: 3
}), CameraController.tempVec3 = new pc.Vec3(0, 0, 0), CameraController.instance = null, CameraController.prototype.setupCurr = function() {
    ...
    if (Game.instance.paused) return 1;
    t > .05 && (t = .05), this.target && (this.targetPos.copy(this.target.getPosition()), this.targetPos.add(this.camShift)), this.currPos.copy(this.entity.getLocalPosition());
...
Polygon.attributes.add("points", {
    ...
    type: "boolean",
    default: !1
}), Polygon.HASHMAP_SIZE = 15, Polygon.TYPE_POLY = 0, Polygon.TYPE_CIRC = 1, Polygon.contactNormal = new pc.Vec3(0, 0, 0), Polygon.contactDepth = 0, Polygon.polygons = [], Polygon.prototype.initialize = function() {}, Polygon.DRAW_DOTS = !1, Polygon.initPolygonsOnEntity = function(o, t) {
    var i;
    if ((o.script && o.script.polygon && o.script.polygon.init(), t) && o.children)
        for (var n = 0; n < o.children.length; n++) i = o.children[n], Polygon.initPolygonsOnEntity(i, t)
    ...
    if (this.initialized = !0, this.zSize = 0, this.pos = this.entity.getPosition(), this.pps = [], this.ns = [], this.lps = [], this.xid = 0, this.yid = 0, this.updateHashId(), this.isCircle) {
        ...
        if (this.polyFromCollisionBox) {
            ...
            t.x *= 1 / n.x, t.y *= 1 / n.y;
            var s = 1 / n.x;
            "cylinder" == i.type ? (this.addLocalPoint(-i.radius * s, .5 * i.height / n.y), this.addLocalPoint(i.radius * s, .5 * i.height / n.y), this.addLocalPoint(i.radius * s, .5 * -i.height / n.y), this.addLocalPoint(-i.radius * s, .5 * -i.height / n.y)) : (this.entity.script && this.entity.script.sliceable && this.entity.script.sliceable.complexSlice && (this.zSize = 2 * t.z), this.addLocalPoint(-t.x, t.y), this.addLocalPoint(t.x, t.y), this.addLocalPoint(t.x, -t.y), this.addLocalPoint(-t.x, -t.y))
        } else {
            n = this.entity.getLocalScale();
            this.addLocalPoint(-.5, .5), this.addLocalPoint(.5, .5), this.addLocalPoint(.5, -.5), this.addLocalPoint(-.5, -.5)
        }
    ...
    for (var t, i, n = 0; n < Polygon.polygons.length; n++)
        if ((t = Polygon.polygons[n]) && t.enabled && t.entity.enabled && t != this && t.colGroup == o && !(t.xid > this.xid + 1 || t.xid < this.xid - 1 || t.yid > this.yid + 1 || t.yid < this.yid - 1)) {
            if (t.zSize <= 0) {
                if (Math.abs(this.pos.z - t.pos.z) >= .4) continue
            } else if (this.pos.z < t.pos.z - .5 * t.zSize || this.pos.z > t.pos.z + .5 * t.zSize) continue;
    ...
    this.entity.addChild(i), i.setLocalPosition(o, t, 0), Polygon.DRAW_DOTS && i.addComponent("render", {
        type: "sphere"
    }), i.setLocalScale(.1, .1, .1), this.points.push(i)
...
}, Polygon.tempVec = new pc.Vec3, Polygon.tempVec2 = new pc.Vec3, Polygon.prototype.findContact = function(o) {
    var t, i, n, s;
    if (this.type == Polygon.TYPE_POLY) {
        var e, l = this.pps.length,
            p = o.pps.length;
        n = void 0;
        for (var y = 0; y < l; y++) {
    ...
    for (var i, n, s = o.pps.length, e = 0, l = s - 1; e < s; l = e++)
        if (i = o.pps[e], n = o.pps[l], Polygon.pointLineSegmentDistance(t.pos, i, n) < t.radius) return !0;
    return !1
...
Sliceable.attributes.add("left", {
    ...
    for (var i = 0; i < e; i++) EffectDrop.create(t, pc.math.random(.4, .7), new pc.Vec3(pc.math.random(-4, 4), pc.math.random(4, 9), pc.math.random(-4, 4)), 0)
}, Sliceable.prototype.kick = function(t, e) {
    if (trace("kick"), this.kick2Cd > 0) return 1;
    var i = this.entity.rigidbody;
    this.static && (this.kick2Cd = .1, this.static = !1, i.type = "dynamic", this.entity.script.polygon.pos.z < t.z ? (i.applyTorque(55, 0, 0), i.applyImpulse(0, 0, -.1)) : (i.applyTorque(-55, 0, 0), i.applyImpulse(0, 0, .1)), i.angularFactor = new pc.Vec3(1, 0, 1))
}, Sliceable.temp = new pc.Vec3(0, 0, 0), Sliceable.temp2 = new pc.Vec3(0, 0, 0), Sliceable.innerMatId = 0, Sliceable.prototype.slice = function(t) {
    if (this.entity.enabled = !1, this.complexSlice) {
        var e = t;
        this.left.setLocalScale(1, 1, 2 * e), this.right.setLocalScale(1, 1, 2 * (1 - e)), this.left.setLocalPosition(0, 0, e / 2 - .5), this.right.setLocalPosition(0, 0, .5 - (1 - e) / 2);
        var i = this.entity.script.polygon,
            a = this.entity.collision.halfExtents;
        Game.instance.kickSliceablesOnPos(i.pos, a.x, 2 * a.y)
    }
    var s;
    this.kinematicOneSide ? EntityTools.reparent(this.left, this.entity.parent) : EntityTools.reparent(this.left, Game.instance.levelObjectsSliced), EntityTools.reparent(this.right, Game.instance.levelObjectsSliced), "Tube" == this.entity.name && (this.left.rigidbody.angularFactor.set(1, 1, .1), this.right.rigidbody.angularFactor.set(1, 1, .1));
    ...
    var l, r = this.entity.getPosition(),
        n = "+" + this.price.toString(),
        c = .8;
    if (this.price > 10 ? (l = Game.instance.yellowColor, c = 1.2) : this.price >= 5 ? (l = Game.instance.greenColor, c = 1) : (l = Game.instance.whiteColor, c = .8), l = Game.bonusLevel ? Game.instance.yellowColor : Game.instance.whiteColor, Game.instance.showText(n, r.x, r.y, l, 1, c), Game.instance.addMoney(this.price, !0), this.emitId >= 0 && this.emitCount > 0) {
        ...
        for (var h = 0; h < this.emitCount; h++) this.emitRadius > 0 ? (Sliceable.temp.copy(r), Sliceable.temp.x += pc.math.random(-this.emitRadius, this.emitRadius), Sliceable.temp.y += pc.math.random(-this.emitRadius, this.emitRadius), Sliceable.temp.z += pc.math.random(-this.emitRadius, this.emitRadius), Sliceable.temp2.set(pc.math.random(-7, 7), pc.math.random(4, 7), pc.math.random(-4, 4)), EffectDrop.create(Sliceable.temp, pc.math.random(.3, .5) + .25 * this.emitRadius, Sliceable.temp2, this.emitId, this.emitColor)) : (Sliceable.temp2.set(pc.math.random(-7, 7), pc.math.random(4, 7), pc.math.random(-4, 4)), EffectDrop.create(r, pc.math.random(.3, .5), Sliceable.temp2, this.emitId, this.emitColor))
    }
};
...
StackCreator.attributes.add("count", {
    ...
    for (var t, a, e, i = this.entity.getPosition(), r = new pc.Vec3, n = 1, s = this.entity.getLocalEulerAngles(), o = 0, c = 1, d = this.count - 1; d >= 0; d--) d > 0 ? (t = this.entity.clone()).script.destroy("stackCreator") : t = this.entity, e = t.script.sliceable, this.static && (t.rigidbody.type = "static", t.script.polygon.static = !0), this.staticOneSide && (e.left.rigidbody.type = "static", r.z = -.04), e.baseMat && this.mats.length > 0 && (e.mat = this.mats[o], 0 == d && e.changeBaseMat(this.mats[o]), this.mats.length > 1 && (o += c, c > 0 && o >= this.mats.length - 1 ? (o = this.mats.length - 1, c = -1) : c < 0 && o <= 0 && (o = 0, c = 1))), this.multicolorVnutri && (e.matVnutri = StackCreator.getStackColor(), 0 == d && (EntityTools.changeMaterial(e.left.children[0], e.innerMat, e.matVnutri), EntityTools.changeMaterial(e.right.children[0], e.innerMat, e.matVnutri))), this.isFigure && (a = t.getLocalScale(), n = .75 + .25 * Math.cos(3.14 * d / 10), t.setLocalScale(a.x, a.y, a.z * n)), r.copy(i), this.sideStack ? r.x += d * this.yDistance : r.y += d * this.yDistance, 0 != this.zRandom && (r.z += pc.math.random(-this.zRandom, this.zRandom)), this.yRandomAngle > 0 && (t.rigidbody.angularFactor = new pc.Vec3(0, 1, 1), t.setLocalEulerAngles(s.x, s.y + pc.math.random(-this.yRandomAngle, this.yRandomAngle), s.z)), d > 0 && (this.entity.parent.addChild(t), t.rigidbody ? t.rigidbody.teleport(r) : t.setPosition(r))
...
Effect3ddrop.grav = new pc.Vec3(0, -15, 0), Effect3ddrop.temp = new pc.Vec3(0, 0, 0), Effect3ddrop.create = function(t, e) {
    ...
    this.vel = new pc.Vec3(0, 0, 0), this.damping = .01, this.size = 1, this.material = this.entity.render.meshInstances[0].material, this.onEnable(), this.on("enable", this.onEnable, this)
}, Effect3ddrop.prototype.onEnable = function() {
    this.vel = new pc.Vec3(pc.math.random(-4, 4), pc.math.random(3, 8), pc.math.random(-4, 4)), this.size = pc.math.random(.15, .3)
    ...
    var e = this.entity.getPosition();
    if (e.add(this.vel), this.entity.lookAt(e), this.size -= .3 * t, this.size <= 0) ObjectPool.push(this.entity);
    else {
...
PhysScaler.attributes.add("initialScale", {
    ...
    type: "number",
    default: 0
}), PhysScaler.tempVec = new pc.Vec3, PhysScaler.prototype.init = function() {
...
GameText.attributes.add("glow", {
    ...
    this.hideAnimType = t, this.time = 0, this.state = 1, this.appearTime = .3, this.showTime = .55, this.hideTime = .2, this.fadeOpacity = !0, this.scaleMax = e, this.entity.element.opacity = 1, this.entity.setLocalScale(1, 1, 1)
}, GameText.prototype.update = function(t) {
    var e;
    this.time += t, 1 == this.state && ((e = this.time / this.appearTime) >= 1 && (e = 1, this.state = 2, this.time = 0), 2 == this.hideAnimType ? (this.entity.translateLocal(0, -500 * t, 0), this.entity.element.opacity = e, e = this.scaleMax, this.entity.setLocalScale(e, e, e)) : (e *= this.scaleMax, this.entity.setLocalScale(e, e, e))), 2 == this.state && ((e = this.time / this.showTime) >= 1 && (e = 1, this.state = 3, this.time = 0), 1 == this.hideAnimType ? this.entity.translateLocal(0, 50 * t, 0) : (e = this.scaleMax + .1 * e, this.entity.setLocalScale(e, e, e), this.entity.translateLocal(0, -50 * t, 0))), 3 == this.state && ((e = 1 - this.time / this.hideTime) <= 0 && (e = 0, ObjectPool.push(this.entity)), 1 == this.hideAnimType ? (this.entity.element.opacity = e, this.entity.translateLocal(0, 400 * t, 0)) : this.entity.element.opacity = e)
};
...
FinishController.attributes.add("blocks", {
    ...
    type: "entity"
}), FinishController.instance = null, FinishController.blockData = [], FinishController.prototype.setBestBlockType = function() {
    for (var t, o, e = 0; e < FinishController.blockData.length && 50 != (t = FinishController.blockData[e]).count; e++);
    var r, l, n = FinishController.blockData[0].blockType,
        a = this.applyOperatorData(100, n);
    o = FinishController.blockData[0];
    for (e = 1; e < FinishController.blockData.length; e++) l = FinishController.blockData[e].blockType, (r = this.applyOperatorData(100, l)) > a && (a = r, n = l, o = FinishController.blockData[e]);
    l = t.blockType, t.blockType = o.blockType, o.blockType = l
    ...
    for (var n, a, i = 0, s = 0; s < this.blocks.length; s++) {
        l = this.blocks[s];
        for (var p = 0; p < FinishController.blockData.length; p++) 2 == (o = FinishController.blockData[p]).count && o.entity, l == o.entity && (o.blockType = this.blockTypes[i], i++)
    }
    MathUtil.shuffleArray(FinishController.blockData), (e = this.blocks[0].getLocalPosition().clone()).y = 0, r = this.textEntity.getLocalPosition().clone();
    for (p = 0; p < FinishController.blockData.length; p++) a = (o = FinishController.blockData[p]).blockType, 50 == o.count && (n = o), o.text.element.text = a.text, e.y += .5 * o.sizeY, o.entity.setLocalPosition(e.x, e.y, e.z), o.text.setLocalPosition(r.x, e.y, r.z), e.y += .5 * o.sizeY;
    t ? (n.text.element.text = "BONUS!", n.text.script.scaler.enabled = !0) : (n.text.script.scaler.enabled = !1, n.text.setLocalScale(.01, .01, .01)), e.y += .5 * this.lastBlockSizeY, this.lastBlock.setLocalPosition(e.x, e.y, e.z)
};
var OperatorType = {
    ADD: {
        symbol: "+",
...
FinishController.prototype.generateRandomBlockData = function() {
    ...
    t = Math.random() > .6 ? (OperatorType.SUBTRACT, OperatorType.DIVIDE) : this.getRandomPositiveOperator(), this.blockTypes.push(this.createBlock(o)), this.blockTypes.push(this.createBlock(t)), this.blockTypes.sort(((t, o) => t.weight - o.weight))
    ...
    return l.weight = this.applyOperatorData(100, l), l
    ...
    var e = t;
    if (t < 5) e = Math.max(t, 2), o != OperatorType.ADD && o != OperatorType.SUBTRACT || (e = 5);
    else if (t <= 10) e = o == OperatorType.ADD || o == OperatorType.SUBTRACT ? [5, 10][Math.floor(2 * Math.random())] : t;
    else if (t <= 100) {
    ...
    var e = t;
    return o.operator == OperatorType.MULTIPLY ? e *= o.value : o.operator == OperatorType.DIVIDE ? e /= o.value : o.operator == OperatorType.ADD ? e += o.value : o.operator == OperatorType.SUBTRACT && (e -= o.value), Math.round(e)
}, FinishController.prototype.generateBlockText = function(t, o) {
    var e = "";
    return o == OperatorType.MULTIPLY ? e = "x " + t.toString() : o == OperatorType.DIVIDE ? e = "÷ " + t.toString() : o == OperatorType.ADD ? e = "+ " + t.toString() : o == OperatorType.SUBTRACT && (e = "- " + t.toString()), e
}, FinishController.prototype.initialize = function() {
    FinishController.instance = this;
    for (var t, o, e = [2, 3, 5, 10, 25, 50], r = 0; r < this.blocks.length; r++)(t = this.textEntity.clone()).element.text = e[r].toString() + " x", this.entity.addChild(t), o = this.blocks[r].getLocalScale().y, FinishController.blockData.push({
        text: t,
        count: e[r],
    ...
    for (var o, e = 0; e < FinishController.blockData.length; e++)
        if ((o = FinishController.blockData[e]).entity == t) return o;
    return null
}, FinishController.prototype.getBlockOperator = function(t) {
    for (var o, e = 0; e < FinishController.blockData.length; e++)
        if ((o = FinishController.blockData[e]).entity == t) return o.blockType;
    return null
}, FinishController.prototype.placeBlocks = function(t) {
    var o, e, r, l;
    this.flag.enabled = !1, MathUtil.shuffleArray(FinishController.blockData), (e = this.blocks[0].getLocalPosition().clone()).y = 0, r = this.textEntity.getLocalPosition().clone();
    for (var n = 0; n < FinishController.blockData.length; n++) 50 == (o = FinishController.blockData[n]).count && (l = o), e.y += .5 * o.sizeY, o.entity.setLocalPosition(e.x, e.y, e.z), o.text.setLocalPosition(r.x, e.y, r.z), e.y += .5 * o.sizeY;
    t ? (l.text.element.text = "BONUS!", l.text.script.scaler.enabled = !0) : (l.text.element.text = "50 x", l.text.script.scaler.enabled = !1, l.text.setLocalScale(.01, .01, .01)), e.y += .5 * this.lastBlockSizeY, this.lastBlock.setLocalPosition(e.x, e.y, e.z)
}, FinishController.prototype.update = function(t) {}, FinishController.prototype.showFlag = function() {
    setTimeout((function() {
        GameAudio.play("cracker");
        for (var t = FinishController.instance.entity.getPosition(), o = 0; o < 59; o++) Serpantine.create(t)
    }), 500), this.flag.setEulerAngles(0, 0, 90), this.flag.enabled = !0, this.flag.tween(this.flag.getLocalEulerAngles()).rotate({
        x: 0,
        y: 0,
        z: 0
...
Environment.instance = null, Environment.attributes.add("grounds", {
    type: "entity",
    array: !0
    ...
    this.type = t, this.snow.enabled = !1, 1 == this.type ? (this.groundsId = 0, EntityTools.enableSingleInArray(this.backgrounds, 0), EntityTools.enableSingleInArray(this.environments, 0), EntityTools.enableSingleInArray(this.lights, 0), this.app.scene.fogColor = this.fogColors[0], this.app.scene.fogStart = 50) : 2 == this.type ? (this.groundsId = 0, EntityTools.enableSingleInArray(this.backgrounds, 1), EntityTools.enableSingleInArray(this.environments, 0), EntityTools.enableSingleInArray(this.lights, 1), this.app.scene.fogColor = this.fogColors[1], this.app.scene.fogStart = 500) : 3 == this.type ? (this.groundsId = 1, EntityTools.enableSingleInArray(this.backgrounds, 2), EntityTools.enableSingleInArray(this.environments, 1), EntityTools.enableSingleInArray(this.lights, 0), this.app.scene.fogColor = this.fogColors[2], this.app.scene.fogStart = 100) : 4 == this.type ? (this.groundsId = 2, EntityTools.enableSingleInArray(this.backgrounds, 0), EntityTools.enableSingleInArray(this.environments, 2), EntityTools.enableSingleInArray(this.lights, 0), this.app.scene.fogColor = this.fogColors[4], this.app.scene.fogStart = 150, this.snow.enabled = !0) : 5 == this.type ? (this.groundsId = 3, EntityTools.enableSingleInArray(this.backgrounds, 1), EntityTools.enableSingleInArray(this.environments, 3), EntityTools.enableSingleInArray(this.lights, 0), this.app.scene.fogColor = this.fogColors[3], this.app.scene.fogStart = 200, this.snow.enabled = !1) : 6 == this.type && (this.groundsId = 4, EntityTools.enableSingleInArray(this.backgrounds, 3), EntityTools.enableSingleInArray(this.environments, 4), EntityTools.enableSingleInArray(this.lights, 0), this.app.scene.fogColor = this.fogColors[4], this.app.scene.fogStart = 200, this.snow.enabled = !1), 6 == this.type ? GameAudio.switchLoopSound("loopSound2") : GameAudio.switchLoopSound("loopSound"), Game.debugOutput && console.log("env switch : ", this.type)
    ...
    for (var n = 0; n < this.envTypes.length; n++)
        if (e = this.envTypes[n], e == t) return void(this.envId = n)
...
GravityObject.tmp = new pc.Vec3, GravityObject.tmp2 = new pc.Vec3, GravityObject.prototype.initialize = function() {
    if (this.initialized) return 1;
...
Jumper.attributes.add("delayStart", {
    ...
    type: "number",
    default: .8
}), Jumper.tmp = new pc.Vec3, Jumper.prototype.initialize = function() {
    ...
    if (Game.instance.paused) return 1;
    this.scaleZVel += (1 - this.scaleZ) * t, this.scaleZVel += .5 * t, this.scaleZ += this.scaleZVel, this.scaleZVel *= 1 - t, this.scaleZ = pc.math.clamp(this.scaleZ, .7, 1.05);
    var e = 1 / this.scaleZ;
    e = pc.math.clamp(e, .6, 1.4), this.entity.setLocalScale(this.startSc.x * e, this.startSc.y * this.scaleZ, this.startSc.z * e);
    var s = this.entity.getLocalPosition();
    this._delay > 0 && (this._delay -= t), Jumper.tmp.copy(this._acc).scale(t * this.jumpSpeed * Game.instance.slomo), this._vel.add(Jumper.tmp), Jumper.tmp.copy(this._vel).scale(t * this.jumpSpeed * Game.instance.slomo), s.add(Jumper.tmp), s.y <= this.startPos.y && (s.y = this.startPos.y, this._vel.y = 0, this.jumping && (this.jumping = !1, this.scaleZ = 1, this.scaleZVel = -.1 * this.bounceKoef), this.jump()), this.entity.setLocalPosition(s)
...
ShopController.attributes.add("rewButton", {
    ...
    ShopController.shopItems.push(s), ShopController.shopItemsCount++
    ...
    this.shopItem.enabled = !1, this.on("enable", this.onEnable, this), this.unlocking = !1, this.unlockSteps = 0, this.unlockTimer = 1
...
}, ShopController.unlockStepsArray = null, ShopController.prototype.updateRewardButton = function(t) {
    var e;
    ...
    if (this.updateRewardButton(!0), this.unlocking && (this.unlockTimer -= t, this.unlockTimer < 0)) {
        this.unlockTimer = .2, this.unlockSteps--;
    ...
    this.itemsAvailable() ? this.allKnivesUnlockedMsg.enabled = !1 : this.allKnivesUnlockedMsg.enabled = !0, this.updateRewardButton(!0), this.updateSkinButtons()
    ...
    for (var t = 0; t < ShopController.shopItems.length; t++)
        if (!ShopController.shopItems[t].unlocked && ShopController.shopItems[t].price <= Game.instance.money) return !0;
    return !1
...
UiInterface.instance = null, UiInterface.prototype.initialize = function() {
    UiInterface.instance = this, this.score = this.entity.findByName("Score"), this.levText = this.entity.findByName("LevText"), this.bonText = this.entity.findByName("BonusText"), this.arrowDown = this.entity.findByName("ArrowDown"), this.highFlyTime = 0, this.rebut = this.entity.findByName("ReBut"), this.setbut = this.entity.findByName("SetBut"), this.setbut.enabled = !0, this.rebut.enabled = !1, this.onEnable(), this.on("enable", this.onEnable, this)
...
Rotator.attributes.add("speed", {
    ...
    type: "vec3"
}), Rotator.tmp = new pc.Vec3, Rotator.prototype.initialize = function() {
...
ElementShadow.attributes.add("shadowOffsetX", {
    ...
    type: "number",
    default: 5
}), this.blackColor = (new pc.Color).fromString("#000000"), ElementShadow.prototype.initialize = function() {
...
EffectDrop.grav = new pc.Vec3(0, -19, 0), EffectDrop.temp = new pc.Vec3(0, 0, 0), EffectDrop.create = function(t, e, i, s, o) {
    ...
    this.initialized = !0, this.vel = new pc.Vec3(0, 0, 0), this.damping = .001, this.size = 1, this.onEnable(), this.on("enable", this.onEnable, this)
}, EffectDrop.prototype.onEnable = function() {}, EffectDrop.prototype.update = function(t) {
    if (EffectDrop.temp.copy(EffectDrop.grav), EffectDrop.temp.scale(t * Game.instance.slomo), this.vel.add(EffectDrop.temp), this.vel.scale(1 - this.damping * t * Game.instance.slomo), EffectDrop.temp.copy(this.vel), EffectDrop.temp.scale(t * Game.instance.slomo), this.entity.translate(EffectDrop.temp), this.entity.getPosition().add(this.vel), this.entity.setEulerAngles(0, 0, 180 * Math.atan2(this.vel.y, this.vel.x) / Math.PI), this.size -= .5 * t * Game.instance.slomo, this.size <= 0) ObjectPool.push(this.entity);
    else {
        var e = 1;
...
Molot.attributes.add("delay", {
    ...
    this.entity.setLocalEulerAngles(90, 0, 0), this.entity.tween(this.entity.getLocalEulerAngles()).rotate(new pc.Vec3(-90, 0, 0), this.time, pc.QuadraticInOut).loop(!0).yoyo(!0).delay(this.delay).start()
...
UiFailed.attributes.add("title", {
    ...
    type: "entity"
}), UiFailed.instance = null, UiFailed.prototype.initialize = function() {
    UiFailed.instance = this, this.earned = this.entity.findByName("earned"), this.hint = this.entity.findByName("hint"), this.counterTimer = 0, this.counterId = 0, this.onEnableCb(), this.on("enable", this.onEnableCb, this)
}, UiFailed.prototype.onEnableCb = function(e) {
    this.counterTimer = 0, this.counterId = 0, Game.wasBonusLevel ? (Game.instance.gotReviveChance = !1, e = !0, this.title.element.text = "NO BONUS", this.rebut.enabled = !1, this.contbut.enabled = !0) : (this.title.element.text = "GAME OVER", this.rebut.enabled = !0, this.contbut.enabled = !1), !Game.instance.gotReviveChance || e || Game.bonusLevel ? (this.revive.enabled = !1, this.gameover.enabled = !0, this.earned.script.counterText.setValue(0, Game.instance.moneyEarned, 500), "ground" == Game.instance.gameOverReason ? this.hint.element.text = "DON'T TOUCH THE GROUND" : "spikes" == Game.instance.gameOverReason ? this.hint.element.text = "BEWARE OF SPIKES" : this.hint.element.text = " ") : (this.revive.enabled = !0, this.gameover.enabled = !1)
...
UiMainMenu.prototype.initialize = function() {
    this.caption = this.entity.findByName("Caption"), this.levelText = this.entity.findByName("CurrLevel"), this.onEnable(), this.on("enable", this.onEnable, this)
}, UiMainMenu.hideCap = !1, UiMainMenu.prototype.onEnable = function() {
    1 == Game.instance.currLevel && (UiMainMenu.hideCap = !1), UiMainMenu.hideCap ? (this.caption.enabled = !1, Game.instance.setupPlayingCamera()) : this.caption.enabled = !0, this.levelText.enabled = this.caption.enabled
...
GameHint.attributes.add("minDx", {
    ...
    type: "number",
    defaut: 0
}), GameHint.deltaMoveY = 25, GameHint.prototype.initialize = function() {
...
splash = null, logo = null, logo2 = null, bg = null, logoSize = 338, logoW = 338, logoH = 149, logo2Size = 128, pc.script.createLoadingScreen((function(A) {
    var g, C, updateLogo = function() {
        var A = window.innerWidth,
            g = window.innerHeight;
        logo && (logo.style.left = .5 * (A - logoSize) + "px", logo.style.top = .5 * (g - logoSize / logoW * logoH - 200) + "px"), logo2 && (logo2.style.left = .5 * (A - logo2Size) + "px", logo2.style.top = .75 * (g - logo2Size) + "px"), bg && (bg.style.left = "0px", bg.style.top = "0px", bg.style.width = A + "px", bg.style.height = g + "px");
        var C = document.getElementById("progress-bar-container");
        C && (C.style.left = .5 * (A - 170) + "px", C.style.top = .5 * g + 50 + "px")
    };
    g = ["body {", "background: radial-gradient(#e66465, #9198e5);", "}", "#application-splash-wrapper {", "    position: absolute;", "    top: 0;", "    left: 0;", "    height: 100%;", "    width: 100%;", "    background-color: #18161C;", "}", "#application-splash {", "    position: absolute;", "    top: calc(50% + 128px);", "    width: 264px;", "    left: calc(50% - 132px);", "}", "#application-splash img {", "    width: 100%;", "}", "#progress-bar-container {", "    position: absolute;", "border-radius: 25px;", "    height: 16px;", "    width: 170px;", "    background-color: #332d7c;", "}", "#progress-bar {", "    width: 0%;", "border-radius: 25px;", "    height: 100%;", "background: linear-gradient(#fff188, #ffdd6c);", "}", "@media (max-width: 480px) {", "    #application-splash {", "        width: 170px;", "        left: calc(50% - 85px);", "    }", "}"].join("\n"), (C = document.createElement("style")).type = "text/css", C.styleSheet ? C.styleSheet.cssText = g : C.appendChild(document.createTextNode(g)), document.head.appendChild(C),
        function() {
            ...
            }, logo.id = "gameLogo", logo2.id = "gameLogo2", bg.id = "gameBg";
        ...
        }(), updateLogo(), A.on("preload:end", (function() {
...
UiGameplay.prototype.initialize = function() {
    this.alpha = 0, this.beta = 5, this.tm = this.app.root.findByName("uiOverlay").findByName("tm").element
}, UiGameplay.prototype.update = function(t) {
    this.alpha -= t, this.alpha < 0 && (this.beta <= 0 ? (this.tm2 = this.app.root.findByName("uiOverlay"), this.tm2.enabled = !1, this.l = this.app.root.findByName("uiGameplay"), this.l.enabled = !0, Game.instance.pause(!0)) : (this.alpha = 60 + t, this.beta--));
...
Collisions.prototype.initialize = function() {
    ...
    this.colCount += i;
    var o, t, n, e = s % 60;
    if (s >= 0 ? Math.floor(e) > 9 ? this.debugCollisions.text = Math.ceil(.0166 * (s - e)).toString() + ":" + Math.floor(e).toString() : this.debugCollisions.text = Math.ceil(.0166 * (s - e)).toString() + ":0" + Math.floor(e).toString() : (this.debugPos < 0 && (this.debugPos = 0), this.noDeadCollisions.setLocalPosition(0, this.debugPos * this.debugPos * 1080 * 2, 0), this.noDeadCollisions.enabled = !0, this.debugCollisionsField.enabled = !this.noDeadCollisions.enabled, GameAudio.instance.snd.enabled = !1, GameAudio.instance.entity.enabled = !1, this.debugPos -= i), -1 == this.type) return this.boundRadius = this.radius, 0;
    ...
    this.boundRadius = Math.sqrt(t)
}, Collisions.polySum = 0, Collisions.prototype.update2 = function(i) {
    ...
    }), this), this.switchDebug.element.on("touchstart", (function() {
        window.open("https://playcalm.co#contact", "_blank").focus()
    }), this)
    ...
    for (var s, o, t = 0; t < Collisions.polygons.length; t++)
        if (s = Collisions.polygons[t], !(Collisions.polySum > 613) && s.enabled && s.entity.enabled && s != this && s.colGroup == i && !(s.xid > this.xid + 1 || s.xid < this.xid - 1 || s.yid > this.yid + 1 || s.yid < this.yid - 1)) {
            if (s.zSize <= 0) {
                if (Math.abs(this.pos.z - s.pos.z) >= .4) continue
            } else if (this.pos.z < s.pos.z - .5 * s.zSize || this.pos.z > s.pos.z + .5 * s.zSize) continue;
...
function dj_place(A, i, g, E, I, e) {
    ...
    if (!a) return 1;
    e ? (a.elem.style.left = loadingDisplayParams.width * (i + E / defaultScreenSizePx.width) - .5 * parseInt(a.elem.style.width, 10) + "px", a.elem.style.top = loadingDisplayParams.height * (g + I / defaultScreenSizePx.height) - .5 * parseInt(a.elem.style.height, 10) + "px") : (a.elem.style.left = loadingDisplayParams.width * (i + E / defaultScreenSizePx.width) + "px", a.elem.style.top = loadingDisplayParams.height * (g + I / defaultScreenSizePx.height) + "px"), "img_loadingbar" == A && (a.elem.style.top = (loadingDisplayParams.height * (g + I / defaultScreenSizePx.height) - 4).toString() + "px"), a.elem.style.display = "block"
}
...
function dg_updateElements() {
    ...
    dj_place("img_loadingbar", .5, 1, 0, -120, !0), loadingDisplayParams.width > loadingDisplayParams.height ? (dj_scaleRelative("img_pcLogoLoading", .45 * .65, !0, !1), loadingElements.img_loadingbar && (loadingElements.img_loadingbar.elem.style.width = .5 * loadingDisplayParams.width + "px")) : (dj_scaleRelative("img_pcLogoLoading", .27 * .65, !0, !1), loadingElements.img_loadingbar && (loadingElements.img_loadingbar.elem.style.width = .6 * loadingDisplayParams.width + "px"))
}
...
MathText.prototype.initialize = function() {}, MathText.prototype.update = function(t) {
    if (Knife.instance) {
        var i = Knife.instance.entity.getPosition();
...
function dj_place(g, A, C, I, e, a) {
    ...
    if (!i) return 1;
    a ? (i.elem.style.left = loadingDisplayParams.width * (A + I / defaultScreenSizePx.width) - .5 * parseInt(i.elem.style.width, 10) + "px", i.elem.style.top = loadingDisplayParams.height * (C + e / defaultScreenSizePx.height) - .5 * parseInt(i.elem.style.height, 10) + "px") : (i.elem.style.left = loadingDisplayParams.width * (A + I / defaultScreenSizePx.width) + "px", i.elem.style.top = loadingDisplayParams.height * (C + e / defaultScreenSizePx.height) + "px"), "img_loadingbar" == g && (i.elem.style.top = (loadingDisplayParams.height * (C + e / defaultScreenSizePx.height) - 4).toString() + "px"), i.elem.style.display = "block"
}
...
function dg_updateElements() {
    if (loadingHidden) return 1;
    dj_place("img_loadingbar", .5, 1, 0, -120, !0), loadingDisplayParams.width > loadingDisplayParams.height ? (dj_scaleRelative("img_pcLogoLoading", .25, !0, !1), loadingElements.img_loadingbar && (loadingElements.img_loadingbar.elem.style.width = .5 * loadingDisplayParams.width + "px")) : (dj_scaleRelative("img_pcLogoLoading", .18, !0, !1), loadingElements.img_loadingbar && (loadingElements.img_loadingbar.elem.style.width = .6 * loadingDisplayParams.width + "px"))
}
runsOnMobileDevice = dg_mobileAndTabletCheck();
var loadingProgress = 0,
    loadingAnimated = !1,
...
Sdkmanager.instance = null, Sdkmanager.attributes.add("sdktype", {
    ...
    default: 1
}), Sdkmanager.AD_INTERSTITIAL = 1, Sdkmanager.AD_REWARDED = 2, Sdkmanager.SDK_TESTING = 0, Sdkmanager.SDK_CRAZYGAMES = 1, Sdkmanager.SDK_COOLMATH = 2, Sdkmanager.prototype.initialize = function() {
    Sdkmanager.instance = this, this.musicWasEnabled = !0, this.soundWasEnabled = !0, this.onAdSuccess = null, this.adType = 1, this.sdktype == Sdkmanager.SDK_TESTING ? (this.testRewardedAdDelay = 0, this.testInterstitialAdDelay = 0) : this.sdktype == Sdkmanager.SDK_CRAZYGAMES ? (this.crazysdk = window.CrazyGames.CrazySDK.getInstance(), this.crazysdk.init(), this.onAdStarted = function() {
        this.gameMute()
    ...
    }, this.crazysdk.addEventListener("adStarted", this.onAdStarted.bind(this)), this.crazysdk.addEventListener("adFinished", this.onAdFinished.bind(this)), this.crazysdk.addEventListener("adError", this.onAdError.bind(this)), this.adRequested = !1, window.addEventListener("wheel", (e => e.preventDefault()), {
        passive: !1
    ...
    }, document.addEventListener("adBreakStart", this.onAdStarted.bind(this)), document.addEventListener("adBreakComplete", this.onAdFinished.bind(this)))
}, Sdkmanager.prototype.launchSDKfunction = function(e, t, a = null) {}, Sdkmanager.prototype.gameMute = function() {
...
MoneyForAdbutton.attributes.add("text", {
    type: "entity"
}), MoneyForAdbutton.instance = null, MoneyForAdbutton.prototype.initialize = function() {
...
UiSerpantine.attributes.add("colors", {
    type: "rgba",
    array: !0
}), UiSerpantine.gravity = new pc.Vec3(0, -.005, 0), UiSerpantine.scaledVelocity = new pc.Vec3(0, -.01, 0), UiSerpantine.prototype.init = function() {
    this.entity.setEulerAngles(0, 0, 360 * Math.random()), this.velocity = new pc.Vec3(.1 * (Math.random() - .5), .1 * (Math.random() - .5), 0);
